...
def check_message(message):
    if message.chat.id not in loggers.keys():
        ...
        file_handler = logging.FileHandler(filename=f"logs/{message.chat.id}.log")
        file_handler.setLevel(logging.INFO)
        loggers[message.chat.id].addHandler(file_handler)
    loggers[message.chat.id].info(f"{message.from_user.id} {message.from_user.first_name} {message.from_user.last_name}: {message.text} \n ({repr(message)})")
    return True